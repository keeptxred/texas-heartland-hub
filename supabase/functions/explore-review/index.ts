...
Deno.serve(async (request) => {
  ...
  if (body.action === "rollback") {
    ...
    for (const revision of revisions ?? []) {
      if (revision.before_snapshot) {
        ...
        const { error } = await client.from("explore_entities").update(values).eq("id", revision.entity_id);
        if (error) return json({ error: error.message, revisionId: revision.id }, 500);
        restored += 1;
      } else {
        const { error } = await client.from("explore_entities").delete().eq("id", revision.entity_id);
        if (error) return json({ error: error.message, revisionId: revision.id }, 500);
        removed += 1;
      }
    }
    ...
    await client.from("explore_import_jobs").update({ status: "rolled_back", completed_at: completedAt }).eq("id", body.jobId);
  ...
  const { data: record, error: recordError } = await client
    ...
    .eq("id", body.recordId)
    .single();
  if (recordError || !record) return json({ error: recordError?.message ?? "Import record not found" }, 404);

  if (body.action === "reject") {
    const reviewedAt = new Date().toISOString();
    const { error } = await client.from("explore_import_records").update({
  ...
  if (entityId) {
    const { data: existing, error } = await client.from("explore_entities").select("*").eq("id", entityId).single();
    if (error || !existing) return json({ error: error?.message ?? "Target entity not found" }, 404);
  ...
  for (let suffix = 2; suffix < 1000; suffix += 1) {
    const { data: match } = await client.from("explore_entities").select("id").eq("slug", slug).maybeSingle();
  ...
  if (entityId) {
    ...
    const { data: updated, error } = await client.from("explore_entities")
    ...
    const { data: inserted, error } = await client.from("explore_entities").insert(entityValues).select("*").single();
  ...
  if (source) {
    const { data: provenanceSource } = await client.from("explore_sources")
      .upsert({
  ...
  const reviewedAt = new Date().toISOString();
  await client.from("explore_import_records").update({
    entity_id: entityId,
