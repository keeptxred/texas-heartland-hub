...
const HEADERS={"User-Agent":"Mozilla/5.0 (compatible; KeepTXRedBot/1.8; +https://keeptxred.com)",Accept:"application/rss+xml,application/xml,text/xml,text/html,*/*"};
...
Deno.serve(async(request)=>{
  ...
  async function upsertAction(bill:any,feed:Feed,item:Item,date:string,committeeId:string|null){
    ...
    const row={bill_id:bill.id,action_date:date,action_sequence:hash(`${feed.key}|${item.link}|${bill.id}`),chamber:feed.chamber,action_code:`tlo-rss-${feed.kind}`,action_text:text.slice(0,1000),normalized_status:status,committee_id:committeeId,source_url:item.link||feed.url,updated_at:now};
    const{data,error}=await db.from("bill_actions").upsert(row,{onConflict:"bill_id,action_date,action_sequence,action_text"}).select("id").single();if(error)throw error;stats.actions_upserted++;
    // Schedule/activity RSS must never own legal chronology; official TLO history does.
    const{error:syncError}=await db.from("bills").update({last_synced_at:now}).eq("id",bill.id);if(syncError)throw syncError;
    return data?.id||null;
  }

  async function upsertOpportunity(bill:any,feed:Feed,item:Item,date:string,actionId:string|null){const mapping:any={filed:["filed",45],passed:["passed",82],meeting:["committee-hearing",68]},mapped=mapping[feed.kind];if(!mapped)return;const{error}=await db.from("legislative_content_opportunities").upsert({bill_id:bill.id,action_id:actionId,event_type:mapped[0],event_date:date,headline:`${bill.bill_identifier}: ${item.title}`.slice(0,500),summary:(item.description||`${feed.key} update from Texas Legislature Online`).slice(0,1600),priority:mapped[1],source_url:item.link||feed.url,dedupe_key:`${sourceKey}:${feed.key}:${bill.id}:${item.link||item.title}`.slice(0,1800),status:"new",metadata:{source:"Texas Legislature Online RSS",feed_key:feed.key,feed_url:feed.url,rss_title:item.title},updated_at:now},{onConflict:"dedupe_key"});if(error)throw error;stats.opportunities_upserted++;}

  async function upsertDocument(bill:any,feed:Feed,item:Item,date:string){const documentUrl=item.link||feed.url,{data:existing}=await db.from("bill_documents").select("id").eq("bill_id",bill.id).eq("document_url",documentUrl).limit(1).maybeSingle();if(existing?.id)stats.documents_existing++;else{const format=/\.pdf(?:$|\?)/i.test(documentUrl)?"pdf":/\.html?/i.test(documentUrl)?"html":null,{error}=await db.from("bill_documents").insert({bill_id:bill.id,document_type:feed.documentType,document_title:(item.title||`${bill.bill_identifier} ${feed.documentType}`).slice(0,500),document_date:date,document_url:documentUrl,file_format:format,source_key:sourceKey,source_record_key:`${feed.key}:${documentUrl}`,legislature_number:bill.legislature_number,session_code:bill.session_code,bill_type:bill.bill_type,bill_number:bill.bill_number,source_html_url:format==="html"?documentUrl:null,source_pdf_url:format==="pdf"?documentUrl:null,metadata:{source:"Texas Legislature Online RSS",feed_key:feed.key,feed_url:feed.url,rss_description:item.description,rss_pub_date:item.pubDate},last_seen_at:now,last_imported_at:now,updated_at:now});if(error&&!String(error.message).toLowerCase().includes("duplicate"))throw error;if(!error)stats.documents_inserted++;}const patch:any={last_synced_at:now};if(feed.documentType==="bill_text")patch.bill_text_url=documentUrl;if(feed.documentType==="analysis")patch.analysis_url=documentUrl;if(feed.documentType==="fiscal_note")patch.fiscal_note_url=documentUrl;const{error}=await db.from("bills").update(patch).eq("id",bill.id);if(error)throw error;stats.bill_links_updated++;}

  try{
    for(const tuple of FEEDS){const feed:Feed={key:tuple[0],url:tuple[1],kind:tuple[2],chamber:tuple[3],documentType:tuple[4]};let response;try{response=await getText(feed.url,25000);}catch(error){warnings.push(`${feed.key}: ${error instanceof Error?error.message:String(error)}`);stats.feeds[feed.key]={status:0,items:0};continue;}if(!response.ok){warnings.push(`${feed.key}: HTTP ${response.status}`);stats.feeds[feed.key]={status:response.status,items:0};continue;}const items=parse(response.text);stats.items+=items.length;stats.feeds[feed.key]={status:200,items:items.length,matched_items:0};
      for(const item of items){let context=`${item.title}\n${item.description}\n${item.link}\n${item.raw}`,billRefs=refs(context);if((feed.kind==="meeting"||feed.kind==="calendar")&&billRefs.length===0&&item.link&&/capitol\.texas\.gov/i.test(item.link)){try{const linked=await getText(item.link,12000);if(linked.ok&&linked.text){stats.linked_pages_fetched++;context+=`\n${linked.text}`;billRefs=refs(context);}}catch(error){warnings.push(`${feed.key} linked page: ${error instanceof Error?error.message:String(error)}`);}}if(!billRefs.length)continue;stats.feeds[feed.key].matched_items++;const date=eventDate(`${item.title} ${item.description}`,item.pubDate),unique=new Map(billRefs.map((ref)=>[`${ref.legislature||""}:${ref.sessionCode||""}:${ref.billType}:${ref.billNumber}`,ref]));
        for(const ref of unique.values()){stats.matched_bill_refs++;const bill=await findBill(ref);if(!bill){stats.missing_bills++;if(samples.length<12)samples.push({feed:feed.key,title:item.title,ref,matched:false});continue;}if(samples.length<12)samples.push({feed:feed.key,title:item.title,bill:bill.bill_identifier,legislature:bill.legislature_number,session:bill.session_code,matched:true});if(feed.kind==="document"){await upsertDocument(bill,feed,item,date);continue;}let committeeId:string|null=null;if(feed.kind==="meeting"){const name=committeeLabel(item,feed.chamber),committee=await matchCommittee(bill,feed.chamber,name);committeeId=committee?.id||null;const{data:existing}=await db.from("bill_committee_history").select("id").eq("bill_id",bill.id).eq("source_url",item.link||feed.url).eq("action_type","hearing").limit(1).maybeSingle();if(!existing?.id){const{error}=await db.from("bill_committee_history").insert({bill_id:bill.id,committee_id:committeeId,chamber:feed.chamber,committee_name:committee?.committee_name||name,action_type:"hearing",action_description:item.title.slice(0,1000),hearing_date:date,sequence:hash(`${feed.key}:${item.link}:${bill.id}`),source_url:item.link||feed.url,updated_at:now});if(error)throw error;stats.committee_events_inserted++;}}const actionId=await upsertAction(bill,feed,item,date,committeeId);await upsertOpportunity(bill,feed,item,date,actionId);}
      }
    }
    await db.from("legislative_sync_runs").update({completed_at:new Date().toISOString(),status:warnings.length?"completed_with_warnings":"completed",records_seen:stats.items,records_changed:stats.documents_inserted+stats.actions_upserted+stats.committee_events_inserted+stats.opportunities_upserted,cursor_after:stats,errors:warnings}).eq("id",run.id);
    return Response.json({ok:true,run_id:run.id,stats,warnings,samples});
  }catch(error){const message=error instanceof Error?error.message:String(error);warnings.push(message);await db.from("legislative_sync_runs").update({completed_at:new Date().toISOString(),status:"failed",records_seen:stats.items,cursor_after:stats,errors:warnings}).eq("id",run.id);return Response.json({ok:false,error:message,stats,warnings,samples},{status:500});}
});
