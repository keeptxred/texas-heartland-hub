...
--
-- Generic display categories such as "Texas News" must not make retired lifestyle
-- or sports discovery classifications indexable on KeepTXRed. This migration extends
-- the durable discovery boundary to Texas Culture, Texas History, Sports,
-- Sports Culture, and Culture & Identity while preserving the existing readiness
-- evidence, repetition, chronology, quarantine, and image checks.
...
WITH article_state AS (
  ...
  SELECT
    ...
    array_remove(ARRAY[
      ...
      CASE
        WHEN NOT state.quarantined
          AND lower(btrim(coalesce(state.discover_category, ''))) IN (
            'texas culture', 'texas history', 'sports', 'sports culture', 'culture & identity'
          )
...
COMMENT ON VIEW public.adsense_cloud_article_readiness IS
  'Internal per-article AdSense audit aligned with the public article readiness gate: KTR display/discovery taxonomy, quality floor 70, source provenance, repetition, chronology, quarantine, and image readiness.';
