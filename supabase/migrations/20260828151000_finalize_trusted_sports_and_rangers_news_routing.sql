-- Final deterministic routing refinements discovered during production audit.
-- 1) A trusted sports publisher/discovery feed is sufficient evidence for KTR
--    Sports even if the headline is shorthand.
-- 2) Texas Rangers law-enforcement stories belong in KTR Texas News, not the
--    Sports lane. Existing contamination/review signals remain intact.
...
begin
  ...
  is_law_enforcement_rangers :=
    ...
    and content_text ~ '(\mdps\M|department of public safety|law enforcement|trooper|troopers|criminal investigation|public safety commission|rangers leadership|chief of the texas rangers|solve[sd]? .* murder|police)';
  ...
  -- Texas Rangers police/DPS stories are legitimate KTR news but never sports.
  if is_law_enforcement_rangers then
    new.target_site := 'keeptxred';
    new.target_section := 'Texas News';
    new.ready_for_rewrite := false;
    new.viral_signals := (coalesce(new.viral_signals, '{}'::jsonb) - 'route_quality_hold' - 'route_quality_reason') || jsonb_build_object(
      'auto_publish_eligible', false,
      'editorial_lane', 'REVIEW',
      'route_quality_reason', 'Texas Rangers law-enforcement story routed to Texas News'
    );
    return new;
  end if;

  -- Trust dedicated sports publishers/discovery feeds even when a headline is
  -- shorthand (for example a player name or team-internal feature).
  if is_known_sports_source then
    new.target_site := 'keeptxred';
    new.target_section := 'Sports';
    new.viral_signals := coalesce(new.viral_signals, '{}'::jsonb) - 'route_quality_hold' - 'route_quality_reason';
  elsif new.target_section = 'Sports' and not has_real_sports_signal then
    new.target_site := 'review';
    new.target_section := 'Unclassified';
    new.ready_for_rewrite := false;
    new.viral_signals := coalesce(new.viral_signals, '{}'::jsonb) || jsonb_build_object(
    ...
    return new;
  elsif has_real_sports_signal and coalesce((new.viral_signals->>'route_quality_hold')::boolean, false) then
    new.target_site := 'keeptxred';
    new.target_section := 'Sports';
    new.viral_signals := coalesce(new.viral_signals, '{}'::jsonb) - 'route_quality_hold' - 'route_quality_reason';
  end if;
