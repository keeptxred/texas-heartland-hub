-- Refine the final route-quality guard so true team/sports publishers are not
-- held merely because a headline uses shorthand. Preserve the explicit
-- law-enforcement Texas Rangers veto and clear only this guard's prior holds.
...
begin
  ...
  is_known_sports_source := source_text ~
    '(texas pro sports|texas sports recruiting|texas sports records|texas rangers|houston astros|houston texans|dallas cowboys|dallas stars|ksat spurs|texags|texas tech athletics|texas longhorns athletics|burnt orange nation|blogging the boys|sports illustrated|espn|mlb\.com|nfl\.com|nba\.com|nhl\.com)';
  ...
  has_real_sports_signal :=
    ...
    or (content_text ~ '\mtexas rangers\M' and content_text ~ '(\mmlb\M|baseball|game|games|pitcher|bullpen|inning|innings|batting|playoff|world series|clubhouse|roster|watch|stream|scores?|highlights?)')
  ...
  -- If a previous version of this guard held a legitimate sports row, clear
  -- only this guard's signals and restore the deterministic KTR Sports route.
