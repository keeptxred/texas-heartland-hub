-- Follow-up precision fixes: ordinary council "votes" are not election stories,
-- and HTeaO headlines are explicitly recognized as food/drink even when the
-- headline omits a generic restaurant/tea-shop noun.
...
begin
  ...
  if is_sports then
    ...
    new.target_section := case
      ...
      when haystack ~ '(hteao|restaurant|\mkitchen\M|barbecue|bbq|pizza|pizzeria|cuisine|taqueria|diner|\mcafe\M|café|tea shop|tea house|boba|coffee shop|coffee chain|bakery|chef|food hall|dining|brewery|taproom)' then 'Food & Drink'
      else 'Texas Life'
    end;
  else
...
where internal_slug is null
  ...
  and (
    lower(title) like '%hteao%'
    or lower(title) ~ '(traffic camera|camera contract)'
  );
