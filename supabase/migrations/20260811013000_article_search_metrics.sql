...
GRANT SELECT ON public.article_search_metrics TO service_role;
GRANT INSERT, UPDATE, DELETE ON public.article_search_metrics TO service_role;

-- The authority dashboard still uses daily_articles for publication depth and
-- freshness, but Search Console metrics now come from the canonical public URL
-- metrics store. This prevents metrics from disappearing when a page is served
-- from the static ARTICLES registry rather than daily_articles.
