-- Tighten newsroom site routing so section assignment is driven by the story
-- headline for election vocabulary, while preserving broad metadata for Texas
-- relevance and hard-news/lifestyle detection.
...
begin
  ...
  if is_sports then
    ...
    new.target_section := case
      ...
      when haystack ~ '(restaurant|\mkitchen\M|barbecue|bbq|pizza|pizzeria|cuisine|taqueria|diner|\mcafe\M|café|tea shop|tea house|boba|coffee shop|coffee chain|bakery|chef|food hall|dining|brewery|taproom)' then 'Food & Drink'
      else 'Texas Life'
    end;
  else
...
-- Re-evaluate only unlinked rows that match the clarified routing vocabulary.
update public.texas_news_feed
set title = title
where internal_slug is null
  and texasdefined_slug is null
  and (
    lower(title) ~ '(traffic camera|camera contract|economic incentives?|pizza|pizzeria|cuisine|tea shop|tea house|boba)'
    or lower(title) ~ '\mcouncil\M.*(approves?|votes?|contract|incentive)'
  );
