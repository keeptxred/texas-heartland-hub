...
COMMENT ON FUNCTION public.article_hero_has_visual_readiness_provenance(text) IS
  'True only for article heroes that passed Cloudflare visual validation or use an explicit authoritative official-graphic exemption. Shared heroes must retain the Cloudflare validation marker.';

COMMENT ON FUNCTION public.enforce_article_hero_visual_readiness() IS
  'Quarantines unvalidated ready hero assignments, preserves prior validated heroes, clears superseded candidates after a validated replacement, and keeps validation history.';
