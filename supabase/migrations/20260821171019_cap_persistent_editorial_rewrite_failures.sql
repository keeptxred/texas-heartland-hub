...
declare
  ...
  v_editorial_failures_7d integer := 0;
  v_limit integer;
begin
  ...
  if p_feed_item_id is not null then
    select count(*) into v_editorial_failures_7d
    from public.ai_rewrite_failures f
    where f.feed_item_id = p_feed_item_id
      and f.failed_at >= now() - interval '7 days'
      and f.failure_reason like 'AI rewrite failed — editorial validation rejected the draft:%';

    if v_editorial_failures_7d >= 6 then
      return 'cooldown';
    end if;
