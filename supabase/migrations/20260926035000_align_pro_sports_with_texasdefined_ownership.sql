-- Align pro-sports discovery with the completed ownership separation.
-- Routine Texas sports belongs on TexasDefined; KTR's legacy sports surface is retired.
-- Contamination remains review-held. Linked/published rows are never rewritten here.
...
-- Re-evaluate only unlinked pro-sports discovery rows under the current ownership rule.
...
comment on function public.guard_texas_pro_sports_discovery_row() is
  'Source-scoped pro-sports guard: valid routine Texas pro sports routes to TexasDefined Sports; contamination stays review-held; linked content is untouched.';
