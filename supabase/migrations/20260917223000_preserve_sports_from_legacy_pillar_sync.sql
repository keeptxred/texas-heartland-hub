...
-- The legacy pillar bridge is deliberately conservative, but a sports article can
-- briefly retain a legacy visible category (for example Elections) after the
-- sports classifier has already marked the row as Sports. Do not let the hourly
-- pillar sync overwrite strong sports signals while the sports taxonomy repair
-- owns the final visible category.
...
BEGIN
  ...
  WHERE a.classifier_version NOT LIKE '%texasdefined-excluded'
    ...
    AND NOT (
      lower(coalesce(d.discover_category, '')) = 'sports'
      OR lower(coalesce(d.image_category, '')) = 'sports'
      OR lower(coalesce(d.kind, '')) LIKE 'sports%'
    )
    AND (
      d.category IS NULL
      OR btrim(d.category) = ''
...
COMMENT ON FUNCTION public.sync_historical_article_categories_from_pillars() IS
  'Synchronizes only legacy newsroom categories from canonical pillars while preserving reviewed/locked taxonomy and any row already carrying strong sports markers in discover_category, image_category, or kind.';
