-- Reconcile the production TLO bill-catalog/schedule contract.
-- This migration intentionally represents the final state only; exploratory
-- parser/normalizer attempts are not replayed.

create schema if not exists private;

-- The 89th Legislature includes two called sessions in addition to 89R.
...
-- Cover the FK/join paths used by public legislative pages and refresh jobs.
...
-- Keep future hearing/calendar notices separate from legal bill chronology.
...
-- TLO URLs carry the authoritative session dimension. Correct a mis-associated
-- bill_id before writing RSS actions/schedule events when duplicate bill numbers
-- exist across regular/called sessions.
...
-- Bill sponsor chamber is determined by bill origin + role, not by the label
-- supplied by a report importer.
...
begin
  ...
  elsif lower(new.sponsor_role) in ('sponsor','cosponsor') then
    new.chamber:=case when v_origin='house' then 'senate' else 'house' end;
  end if;
  return new;
end;
$$;
...
-- Immediate aliases cover formal-name variants and the seven malformed accented
-- labels emitted by TLO static filed-report pages. The scheduled normalizer remains
-- the canonical bulk identity resolver.
...
begin
  ...
  loop
    perform public.upsert_bidirectional_authority_relationship(
      'bill',r.bill_key,'representative',r.target_key,r.sponsor_role,
...
-- Bounded normalization prevents one 12k-bill transaction from exceeding the
-- database statement timeout. Repeated invocations are restart-safe.
...
begin
  ...
  create temporary table _sponsor_aliases(
    old_name text not null,chamber text not null,full_name text not null,canonical_slug text not null,
    district text,external_legislator_id text not null,primary key(old_name,chamber)) on commit drop;
  ...
  where nullif(trim(x.old_name),'') is not null and lower(trim(x.chamber)) in ('house','senate')
    and nullif(trim(x.full_name),'') is not null and nullif(trim(x.canonical_slug),'') is not null
  ...
  where b.legislature_number=89 and (
    bs.sponsor_name is distinct from a.full_name or bs.sponsor_slug is distinct from a.canonical_slug
    or bs.chamber is distinct from a.chamber or bs.district is distinct from a.district
  ...
  join _sponsor_aliases a on lower(trim(bs.sponsor_name))=lower(trim(a.old_name)) and lower(coalesce(bs.chamber,b.chamber,''))=a.chamber
  where bs.sponsor_name is distinct from a.full_name or bs.sponsor_slug is distinct from a.canonical_slug
    or bs.chamber is distinct from a.chamber or bs.district is distinct from a.district
    or bs.external_legislator_id is distinct from a.external_legislator_id;

  delete from public.bill_sponsors bs using _candidate_sponsors c
  where bs.id=c.id and exists(
    select 1 from public.bill_sponsors x where x.id<>bs.id and x.bill_id=bs.bill_id
  ...
  update public.bill_sponsors bs set sponsor_name=c.full_name,sponsor_slug=c.canonical_slug,chamber=c.chamber,
    district=c.district,external_legislator_id=c.external_legislator_id
  from _candidate_sponsors c where bs.id=c.id;
  get diagnostics v_updated=row_count;

  delete from public.authority_relationships ar
  where ar.is_manual=false and ar.relationship_type in ('author','coauthor','sponsor','cosponsor','primary author','primary sponsor','sponsor-district')
...
-- Remove obsolete automated sponsor edges left by earlier malformed labels.
delete from public.authority_relationships ar
where ar.is_manual=false
  and coalesce(ar.evidence->>'source','')='official-sponsor-record'
