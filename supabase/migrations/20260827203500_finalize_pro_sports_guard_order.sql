-- Ensure the source-specific pro-sports guard runs after the generic site router.
-- PostgreSQL fires same-kind triggers alphabetically by trigger name, so the
-- zz_ prefix lets the final allowlist/quarantine decision win deterministically.
...
begin
  ...
  if is_allowed then
    ...
    if coalesce((new.viral_signals->>'source_contamination')::boolean, false) is true and new.internal_slug is null then
      ...
      new.viral_signals := coalesce(new.viral_signals, '{}'::jsonb) - 'source_contamination' - 'auto_publish_eligible' - 'editorial_lane' - 'exclusion_reason';
    end if;
  else
...
create trigger zz_guard_texas_pro_sports_discovery_row
...
comment on trigger zz_guard_texas_pro_sports_discovery_row on public.texas_news_feed is
  'Runs after generic routing so Texas Pro Sports allowlist/quarantine decisions are final.';
