-- Keep coverage-gap escalation strictly bounded to the current top-N candidate set.
-- Earlier sync versions capped inserts but left older still-eligible alerts open,
-- allowing the open alert count to grow past p_limit. This remains alert-only.
...
begin
  ...
  -- Resolve every coverage-gap alert that is not in the current top-N candidate
  -- set. This enforces p_limit as a true open-alert cap instead of only an insert cap.
  with current_top as (
    select 'coverage-gap-' || g.id::text as incident_key
    from public.news_coverage_gaps g
    where g.gap_reason in (
        'article_generation_or_publish_gap',
        'held_for_corroboration',
    ...
    where a.status='open'
      ...
      and not exists (select 1 from current_top c where c.incident_key=a.incident_key)
    returning 1
...
comment on function public.sync_coverage_gap_alerts(integer,integer,integer) is
  'Maintains a strictly bounded top-N alert set for overdue high-priority newsroom gaps; alert-only and never publishes content.';
