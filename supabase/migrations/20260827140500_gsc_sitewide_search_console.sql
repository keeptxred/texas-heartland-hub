...
CREATE INDEX IF NOT EXISTS gsc_page_daily_metrics_date_idx
  ON public.gsc_page_daily_metrics (metric_date DESC);
CREATE INDEX IF NOT EXISTS gsc_page_daily_metrics_path_idx
  ON public.gsc_page_daily_metrics (path, metric_date DESC);

COMMENT ON TABLE public.gsc_page_daily_metrics IS
  'Daily Search Console page performance for canonical KeepTXRed URLs. Server-only; retains history beyond third-party SEO tooling.';
...
CREATE UNIQUE INDEX IF NOT EXISTS gsc_url_inspection_path_idx
  ON public.gsc_url_inspection (path);
CREATE INDEX IF NOT EXISTS gsc_url_inspection_verdict_idx
  ON public.gsc_url_inspection (verdict, inspected_at DESC);

COMMENT ON TABLE public.gsc_url_inspection IS
  'Latest Google Search Console URL Inspection state for canonical KeepTXRed sitemap URLs. Server-only.';
