-- Route a final set of high-confidence topics observed in the fresh review queue.
-- Ambiguous weather, obituary, medical, development, and campus items remain held.
...
begin
  ...
  if is_sports then
    ...
    new.target_section := case
      when haystack ~ '(big bend|hunter education|tpwd|botanical garden|state park|national park|nature center|public park|park trail|undeveloped parks|trail|camping|hiking|lake|reservoir|river|fishing|wildlife|alligator|zoo|aquarium|bat experience|travel|tourism|road trip|attraction|airport)' then 'Explore'
...
where created_at >= now() - interval '24 hours'
  ...
  and lower(title) ~ '(nyse texas|\mtxse\M|stock exchange|financially struggling|water utilities|water demands|dam and spillway|clear the shelters|animal services|dogust|cultural center|exhibitions?|baby names|sister cities|dolly parton|watermelon capital|big bend|hunter education|tpwd)';
