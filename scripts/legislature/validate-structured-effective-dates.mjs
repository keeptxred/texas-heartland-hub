...
const requiredMigrationPatterns = [
  ...
  /'HB 2'/, /'HB 120'/, /'SB 7'/, /'SB 9'/, /'SB 38'/, /'SB 568'/,
  /'HB 1393'/, /'SB 5'/, /'HB 9'/, /'no_effect'/, /'conditional'/,
];
for (const pattern of requiredMigrationPatterns) if (!pattern.test(migration)) throw new Error(`Structured effective-date migration is missing ${pattern}`);

for (const identifier of ['HB 247','HB 1399','HB 2508','SB 467','SB 2155']) if (!expansion.includes(`'${identifier}'`)) throw new Error(`Structured effective-date expansion is missing ${identifier}.`);
...
for (const identifier of ['HB 140','HB 2789','HB 2844','SB 293','SB 1150']) if (!reviewedComplex.includes(`'${identifier}'`)) throw new Error(`Reviewed complex effective-date coverage is missing ${identifier}.`);
...
for (const identifier of ['HB 3810','SB 1738','SB 1786','SB 2361']) if (!followup.includes(`'${identifier}'`)) throw new Error(`Follow-up structured effective-date coverage is missing ${identifier}.`);
...
for (const identifier of ['HB 4488','HB 5033']) if (!statewideFollowup.includes(`'${identifier}'`)) throw new Error(`Statewide follow-up effective-date coverage is missing ${identifier}.`);
...
for (const identifier of ['HB 3250','HB 3689','SB 1036','SB 22']) if (!finalStatewide.includes(`'${identifier}'`)) throw new Error(`Final statewide effective-date coverage is missing ${identifier}.`);
...
const localIds = ['HB 5651','HB 5655','HB 5658','HB 5677','HB 5680','HB 5682','SB 3037','SB 3047','SB 3048','SB 3056'];
for (const identifier of localIds) {
  if (!localClosure.includes(`'${identifier}'`)) throw new Error(`Local district effective-date closure is missing ${identifier}.`);
}
if (!/date '2025-09-01'/.test(localClosure)) throw new Error('Local district Acts must retain the September 1, 2025 effective date.');
