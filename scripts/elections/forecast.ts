...
for (const race of races) {
  ...
  generatedByRace.set(race.id, { id: `forecast-${race.id}`, slug: `${race.slug}-forecast`, electionCycleId: race.electionCycleId, raceId: race.id, title: `${race.name} forecast`, status: "active", rating: result.rating, confidenceLevel: result.confidenceLevel, candidateProbabilities: result.candidateProbabilities, projectedMargin: result.expectedMargin, projectedWinnerCandidateId: result.projectedWinnerCandidateId, updatedAt: timestamp, createdAt: timestamp, publishedAt: timestamp, verificationStatus: "verified", publicationStatus: "published", dataAsOf: timestamp, source: { sourceId: MODEL_SOURCE_ID, sourceName: "Keep TX Red Election Forecast Model", sourceType: "forecast_provider", sourceUrl: METHOD_URL } });
}
