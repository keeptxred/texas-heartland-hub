...
const candidateRoutePath = path.join(ROOT, "src/routes/elections.candidates.tsx");
const candidateRoute = await readFile(candidateRoutePath, "utf8");
...
const socialImagePath = path.join(ROOT, "public/images/elections/election-central-social.jpg");
try {
  await access(socialImagePath);
} catch {
  blockers.push("Default Election Central social image is missing from public/images/elections/election-central-social.jpg.");
}
...
for (const file of electionSourceFiles) {
  ...
  if (matches.length > 0) {
    ...
    warnings.push(`${relative} contains ${matches.length} unfinished/mock/demo/placeholder marker(s).`);
  }
}
