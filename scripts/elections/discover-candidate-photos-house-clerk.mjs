...
const REPORT_PATH = path.join(ROOT, "artifacts/elections/candidate-photo-house-clerk-report.json");
...
await runPool(queue, CONCURRENCY, async (candidate) => {
  try {
    const entry = await discoverFromHouseClerk(candidate);
    if (entry) {
      byId.set(candidate.id, entry);
      discoveries.push(entry);
    } else {
      failures.push({ candidateId: candidate.id, name: candidate.fullName, reason: "No validated House Clerk portrait found." });
    }
  } catch (error) {
...
console.log(`House Clerk discovery added ${discoveries.length} verified portrait(s).`);

async function discoverFromHouseClerk(candidate) {
  const query = `site:clerk.house.gov/members \"${candidate.fullName}\" Texas`;
  const links = await duckDuckGoSearch(query);
  for (const url of links) {
    if (!/^https:\/\/clerk\.house\.gov\/members\//i.test(url)) continue;
    ...
    for (const image of extractImages(html, response.url || url)) {
      ...
      return {
        ...
        altText: `Official U.S. House portrait of ${candidate.fullName}`,
        credit: "Office of the Clerk, U.S. House of Representatives",
        license: null,
        permissionBasis: "Official U.S. House member portrait published by the Office of the Clerk and used for informational candidate identification with source attribution.",
        usageStatus: "approved",
        discoveredAt: new Date().toISOString(),
        discoveryMethod: "official-house-clerk-member-profile",
        discoverySource: "clerk.house.gov",
      };
    }
  }
  return null;
...
function looksLikeMemberPortrait(image, candidate) {
  ...
  if (/photo of|portrait|member|headshot|profile/.test(haystack)) return true;
