import { describe, expect, it } from "vitest";
import { GOVERNMENT_HISTORY_AUTHORITY_PAGES } from "./texas-government-history-authority";

const AUTHORITATIVE_SOURCE_HOSTS = new Set([
  "capitol.texas.gov",
  "gov.texas.gov",
  "house.texas.gov",
  "lrl.texas.gov",
  "senate.texas.gov",
  "statutes.capitol.texas.gov",
  "tlc.texas.gov",
  "www.lrl.texas.gov",
  "www.ltgov.texas.gov",
  "www.sos.state.tx.us",
  "www.tsl.texas.gov",
  "www.txcourts.gov",
]);

function pageWordCount(page: (typeof GOVERNMENT_HISTORY_AUTHORITY_PAGES)[number]) {
  const text = [
    page.title,
    page.intro,
    page.shortAnswer,
    ...page.timeline.flatMap((item) => [item.event, item.meaning]),
    ...page.sections.flatMap((section) => [section.heading, ...section.paragraphs]),
    ...page.faqs.flatMap((faq) => [faq.question, faq.answer]),
  ].join(" ");
  return text.trim().split(/\s+/).filter(Boolean).length;
}

describe("Texas government history authority pages", () => {
  it("keeps every published authority page substantive", () => {
    for (const page of GOVERNMENT_HISTORY_AUTHORITY_PAGES) {
      expect(page.timeline.length, page.slug).toBeGreaterThanOrEqual(8);
      expect(page.sections.length, page.slug).toBeGreaterThanOrEqual(5);
      expect(page.faqs.length, page.slug).toBeGreaterThanOrEqual(3);
      expect(page.sources.length, page.slug).toBeGreaterThanOrEqual(5);
      expect(page.relatedLinks.length, page.slug).toBeGreaterThanOrEqual(5);
      expect(pageWordCount(page), page.slug).toBeGreaterThanOrEqual(850);
    }
  });

  it("uses authoritative Texas institutional and archival source families", () => {
    for (const page of GOVERNMENT_HISTORY_AUTHORITY_PAGES) {
      for (const source of page.sources) {
        const host = new URL(source.href).hostname;
        expect(AUTHORITATIVE_SOURCE_HOSTS.has(host), `${page.slug}: ${host}`).toBe(true);
      }
    }
  });

  it("connects history back to current government authority", () => {
    for (const page of GOVERNMENT_HISTORY_AUTHORITY_PAGES) {
      expect(
        page.relatedLinks.some((link) => link.href === "/texas-government" || link.href.startsWith("/texas-government/")),
        page.slug,
      ).toBe(true);
    }
  });

  it("uses unique slugs, SEO titles and descriptions", () => {
    const slugs = GOVERNMENT_HISTORY_AUTHORITY_PAGES.map((page) => page.slug);
    const titles = GOVERNMENT_HISTORY_AUTHORITY_PAGES.map((page) => page.seoTitle);
    const descriptions = GOVERNMENT_HISTORY_AUTHORITY_PAGES.map((page) => page.description);
    expect(new Set(slugs).size).toBe(slugs.length);
    expect(new Set(titles).size).toBe(titles.length);
    expect(new Set(descriptions).size).toBe(descriptions.length);
  });
});
