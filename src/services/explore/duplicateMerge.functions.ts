...
async function loadEntitySummaries(
  ...
  for (const pt of profileTables) {
    const { data: rows } = await admin.from(pt).select("entity_id").in("entity_id", ids);
...
export const mergeExploreDuplicateCandidateFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MergeCandidateResult> => {
    ...
    const loserId = data.survivorId === cand.entity_a_id ? cand.entity_b_id : cand.entity_a_id;
    ...
    const { data: rpcData, error: rpcErr } = await admin.rpc("explore_merge_duplicate_candidate", {
