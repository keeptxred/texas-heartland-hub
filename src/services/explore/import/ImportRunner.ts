...
export class ImportRunner {
  ...
  async run(importer: BaseImporter, context: ImportContext): Promise<ImportRunResult> {
    ...
    try {
      await this.updateJob(context.jobId, {
        status: "running",
      ...
      for (const raw of parsed) {
        ...
        try {
          ...
          const action =
            previous?.checksum === checksum ? "unchanged" : previous ? "update" : "insert";
          statistics[
            action === "insert" ? "inserted" : action === "update" ? "updated" : "unchanged"
          ] += 1;
          records.push({
            externalId: normalized.externalId,
            action,
  ...
  private async updateSourceHealth(sourceId: string, success: boolean): Promise<void> {
    ...
    if (!success) {
      ...
      await this.client
        .from("explore_import_sources" as never)
        .update({ consecutive_failures: failures } as never)
        .eq("id", sourceId);
    }
  }
}
