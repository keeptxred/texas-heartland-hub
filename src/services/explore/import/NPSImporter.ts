...
export class NPSImporter extends BaseImporter<JsonRecord> {
  ...
  async normalize(record: JsonRecord, context: ImportContext): Promise<ImportEntityDraft> {
    const draft = normalizeFeature(record, {
      entityType: "national_park",
      sourceUrl: context.source.endpoint,
      idFields: ["parkCode", "id", "url"],
      nameFields: ["fullName", "name", "title"],
      descriptionFields: ["description", "designation"],
      taxonomy: ["parks", "national-park-service"],
    });
    ...
    return {
      ...draft,
      latitude: Number.isFinite(latitude) ? latitude : draft.latitude,
      longitude: Number.isFinite(longitude) ? longitude : draft.longitude,
    };
  }
}
