...
export class HistoricalCommissionImporter extends BaseImporter<JsonRecord> {
  ...
  async normalize(record: JsonRecord, context: ImportContext): Promise<ImportEntityDraft> {
    return normalizeFeature(record, { entityType: "historic_site", sourceUrl: context.source.endpoint, idFields: ["atlas_number", "resource_id", "id", "OBJECTID"], nameFields: ["resource_name", "site_name", "name", "title"], descriptionFields: ["historical_text", "description", "summary"], taxonomy: ["history", "texas-historical-commission"] });
  }
}
