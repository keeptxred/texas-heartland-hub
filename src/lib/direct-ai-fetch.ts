...
async function directGeminiImage(body: OpenAiImageBody, apiKey: string, signal?: AbortSignal | null): Promise<Response> {
  const prompt = body.prompt?.trim() || (body.messages ?? []).map((m) => textFromContent(m.content)).filter(Boolean).join("\n\n").trim();
  if (!prompt) return Response.json({ error: { message: "Image request contained no prompt" } }, { status: 400 });
  ...
  try { payload = JSON.parse(text) as GeminiInteraction; }
  catch { return Response.json({ error: { message: "Gemini image API returned invalid JSON" } }, { status: 502 }); }
  const outputs = (payload.steps ?? []).filter((step) => step.type === "model_output");
  const image = outputs.flatMap((step) => step.content ?? []).reverse().find((part) => part.type === "image" && typeof part.data === "string" && part.data.length > 0);
  if (!image?.data) {
    const failure = (payload.steps ?? []).find((step) => step.error?.message)?.error?.message;
    return Response.json({ error: { message: failure || `Gemini image API returned no image (${payload.status || "unknown status"})` } }, { status: 502 });
  }
  return Response.json({ data: [{ b64_json: image.data }], provider: "google-gemini-direct", model });
}

async function directGeminiVision(body: OpenAiCompatBody, apiKey: string, signal?: AbortSignal | null): Promise<Response> {
  const messages = Array.isArray(body.messages) ? body.messages : [];
  const system = messages.filter((m) => m.role === "system").map((m) => textFromContent(m.content).trim()).filter(Boolean).join("\n\n");
  const conversational = messages
    .filter((m) => m.role !== "system")
    .map((m) => ({ role: m.role === "assistant" ? "model" : "user", parts: partsFromContent(m.content) }))
  ...
  const response = await nativeFetch(endpoint, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        ...
        ...(body.response_format?.type === "json_schema" && body.response_format.json_schema ? { responseJsonSchema: body.response_format.json_schema } : {}),
  ...
  catch { return Response.json({ error: { message: "Gemini returned invalid JSON" } }, { status: 502 }); }
  const content = payload.candidates?.[0]?.content?.parts?.map((part) => part.text || "").join("").trim();
  if (!content) return Response.json({ error: { message: "Gemini returned an empty response" } }, { status: 502 });
  return Response.json({ choices: [{ message: { role: "assistant", content } }], provider: "google-gemini-direct", model });
}

async function directCloudflareText(body: OpenAiCompatBody, credentials: { accountId: string; apiToken: string }, signal?: AbortSignal | null): Promise<Response> {
  const messages = (body.messages ?? [])
    .map((message) => ({ role: message.role === "assistant" ? "assistant" : message.role === "system" ? "system" : "user", content: textFromContent(message.content) }))
    .filter((message) => message.content.trim().length > 0);
  if (!messages.length) return Response.json({ error: { message: "AI text request contained no prompt" } }, { status: 400 });
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    const response = await nativeFetch(endpoint, {
      ...
      body: JSON.stringify({ messages: retryInstruction ? [...messages, retryInstruction] : messages, response_format: body.response_format ?? { type: "json_object" }, max_tokens: maxTokens, temperature: attempt === 1 ? 0.25 : 0.1 }),
      signal: signal ?? undefined,
    });
    const text = await response.text();
    let payload: CloudflareAiResponse | null = null;
    try { payload = JSON.parse(text) as CloudflareAiResponse; }
    catch { lastFailure = `Cloudflare Workers AI ${response.status}: provider envelope was invalid JSON`; continue; }
    if (!response.ok || payload?.success === false) {
      ...
      if (response.status === 429 && /daily free allocation|used up.*neurons/i.test(detail)) return Response.json({ error: { message: lastFailure } }, { status: 429 });
      if (response.status >= 400 && response.status < 500 && response.status !== 408 && response.status !== 429) return Response.json({ error: { message: lastFailure } }, { status: response.status || 502 });
      continue;
    }
    const content = normalizeJsonContent(payload?.result?.response);
    if (content) return Response.json({ choices: [{ message: { role: "assistant", content } }], provider: "cloudflare-workers-ai", model, attempts: attempt });
...
async function directCloudflareImage(body: OpenAiImageBody, credentials: { accountId: string; apiToken: string }, signal?: AbortSignal | null): Promise<Response> {
  const prompt = body.prompt?.trim() || (body.messages ?? []).map((m) => textFromContent(m.content)).filter(Boolean).join("\n\n").trim();
  if (!prompt) return Response.json({ error: { message: "Image request contained no prompt" } }, { status: 400 });
  ...
  if (!response.ok) return Response.json({ error: { message: `Cloudflare image generation failed (${response.status})` } }, { status: response.status });
  const contentType = response.headers.get("content-type") || "";
  if (contentType.includes("application/json")) {
    const payload = (await response.json()) as CloudflareAiResponse;
    const image = payload?.result?.image;
    if (!image) return Response.json({ error: { message: "Cloudflare image generation returned no image" } }, { status: 502 });
...
export function installDirectAiFetch(): void {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url !== INTERNAL_CHAT && url !== INTERNAL_IMAGES) return Response.json({ error: { message: `Unsupported internal AI endpoint: ${url}` } }, { status: 501 });

    let body: OpenAiCompatBody | OpenAiImageBody;
    try { body = JSON.parse(typeof init?.body === "string" ? init.body : "{}") as OpenAiCompatBody | OpenAiImageBody; }
    catch { return Response.json({ error: { message: "Invalid AI request JSON" } }, { status: 400 }); }
    ...
    if (hasImageInput(chatBody)) {
      if (!geminiKey) return Response.json({ error: { message: "Image validation provider is not configured" } }, { status: 503 });
      return directGeminiVision(chatBody, geminiKey, init?.signal);
    }
    if (!cf) return Response.json({ error: { message: "Cloudflare Workers AI text provider is not configured" } }, { status: 503 });
    return directCloudflareText(chatBody, cf, init?.signal);
  }) as typeof globalThis.fetch;

  installed = true;
  if (cf) console.info(`[AI] direct text provider = Cloudflare Workers AI (${process.env.AI_REWRITE_MODEL_CF || CLOUDFLARE_TEXT_MODEL})`);
  else console.warn("[AI] Cloudflare Workers AI credentials missing; text AI calls will fail closed");
}
