...
import {
  POLICY_TRACKER_WAVE13_UPGRADES,
  WAVE13_INDEXABLE_POLICY_TRACKER_SLUGS,
} from "@/data/policy-tracker-wave13-upgrades";
import {
...
const EXPECTED = [
  "assisted-suicide",
  "charter-schools",
  "consumable-hemp-cannabis",
] as const;

describe("wave 13 policy tracker readiness", () => {
  it("keeps the publication cohort explicit", () => {
    expect([...WAVE13_INDEXABLE_POLICY_TRACKER_SLUGS].sort()).toEqual([...EXPECTED].sort());
  });

  it.each(EXPECTED)("%s genuinely clears the unchanged canonical tracker gate", (slug) => {
    const tracker = POLICY_TRACKER_WAVE13_UPGRADES[slug];
