...
function safetyRetryScene(prompt: string): string {
  ...
  if (/\b(medicaid|hospital reimbursement|hospital funding|supplemental funding|healthcare funding)\b/.test(text)) {
    return "A real Texas hospital administration or revenue-cycle office where anonymous staff review reimbursement folders, claim paperwork, payment-reconciliation sheets, calculators, and medical-billing workstations. A clinical corridor may be visible beyond the office. No patient information, agency names, logos, dollar amounts, or readable text.";
  }
  if (/\b(affordability|cost of living|property tax|home insurance|utility bills?|grocery bills?|rent|mortgage)\b/.test(text)) {
    return "A real Texas household kitchen table during a budget review, with property-tax and home-insurance paperwork, utility bills, grocery receipts, rent or mortgage paperwork, a calculator, pen, and ordinary household keys. All words, numbers, logos, and account details are unreadable. No politicians, campaign material, or capitol symbolism.";
  }
  if (/\b(stock exchange|primary listings?|corporate listings?|market listing|listed shares?)\b/.test(text)) {
    return "A modern Dallas financial-market listing venue with a brass market-opening bell, neutral electronic market boards, listing folders, financial paperwork, and anonymous finance workers. No company names, ticker symbols, logos, exchange branding, or readable text; avoid recognizable New York or Chicago exchange interiors.";
  }
  if (/\b(civil rights group|community organizing|immigrant rights|immigration organizing|know your rights|lupe)\b/.test(text)) {
    return "A South Texas community-center organizing meeting with anonymous young adult volunteers assembling know-your-rights packets, clipboards, phone-bank sheets, and outreach materials around folding tables. Faces are incidental, and all text, names, logos, and organization branding are unreadable. The visible activity is community organizing, not a generic protest crowd.";
  }
  if (/\b(social media|online post|posts about|posted about|condemn posts|doubles down)\b/.test(text)) {
    return "An over-the-shoulder documentary view of an anonymous adult reviewing a social-media post on a smartphone near a Texas university campus. The interface is visibly social media but every username, sentence, caption, notification, logo, and account detail is unreadable. Anonymous college-age students appear in the background; no fabricated quote or public-figure likeness.";
  }
...
export function imageValidationDomainGuidance(subject: SubjectExtract): string {
  ...
  if (subject.domain === "politics") {
    return `${PRIMARY_SUBJECT_RULE} For politics or public-policy stories, do NOT require a recognizable likeness of a named politician, the exact date, the exact venue, a specific broadcast or interview, a press conference, or any other historically exact scene. A believable photorealistic government setting or policy-impact setting is valid only when it directly depicts the concrete policy target or action in the story. For affordability, reimbursement, funding, tax, spending, or benefit-policy stories, a concrete household or institutional finance scene showing the affected real-world costs, billing, claims, or payment administration IS a valid direct match even when no politician is visible; generic capitol or campaign scenes are not. For stories whose defining action is a public online post, a real social-media viewing scene tied to the affected institution or community may represent that action when all post text and usernames are unreadable and no fabricated quotation is shown. Prefer anonymous or non-identifiable people. Reject unrelated generic government imagery, location-only skylines, fabricated readable text or logos, and recognizable faces presented as the named politician unless independently verified.`;
  }
  ...
  const financeText = `${subject.title} ${subject.concreteSubject}`.toLowerCase();
  if (/\b(stock exchange|primary listing|corporate listing|market listing)\b/.test(financeText)) {
    return `${PRIMARY_SUBJECT_RULE} For a stock-exchange primary-listing story, a believable modern exchange or listing-venue scene IS a valid direct match when the listing activity itself is visually obvious through a market-opening bell, listing paperwork, market boards, or equivalent physical exchange activity. It does not need to display a readable ticker, company logo, or exchange logo. Reject a recognizable unrelated exchange floor, generic office, or generic finance symbolism that does not show listing activity.`;
  }
