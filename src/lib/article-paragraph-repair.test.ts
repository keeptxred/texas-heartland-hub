...
describe("article paragraph repair", () => {
  ...
  it("repairs malformed intro and section strings before rendering", () => {
    const introSentence = "Austin remains a major dining destination in Central Texas, with visitors comparing restaurants before deciding where to stop.";
    const sectionSentence = "Lockhart also draws diners from across the state, and its barbecue history gives readers useful context when comparing destinations.";
    const introWall = Array.from({ length: 12 }, (_, index) => `${introSentence} Intro detail ${index + 1} adds context.`).join(" ");
    const sectionWall = Array.from({ length: 12 }, (_, index) => `${sectionSentence} Section detail ${index + 1} adds context.`).join(" ");
    const body = dedupeArticleBody({
      intro: [introWall],
      sections: [{ heading: "How the destinations compare", paragraphs: [sectionWall] }],
      faq: [],
      sources: [],
    });

    expect(body.intro && body.intro.length).toBeGreaterThan(1);
    expect(body.sections?.[0]?.paragraphs?.length).toBeGreaterThan(1);
  });
