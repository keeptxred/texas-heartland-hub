...
describe("featured image model routing", () => {
  it("uses FLUX 2 Klein as the quality default with Schnell as an API fallback", () => {
    ...
    expect(CLOUDFLARE_IMAGE_QUALITY_MODEL).toBe("@cf/black-forest-labs/flux-2-dev");
  ...
  it("uses model-aware FLUX 2 multipart prompt-adherence controls and preserves supported Schnell fallback fields", () => {
    ...
    expect(cloudflareSource).toContain('if (model === CLOUDFLARE_IMAGE_QUALITY_MODEL) form.append("steps", "25")');
