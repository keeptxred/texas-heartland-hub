...
describe("featured image model routing", () => {
  it("uses quota-safe Klein 4B by default and keeps Klein 9B for quality escalation and fallback", () => {
    ...
    expect(cloudflareSource).not.toContain("@cf/black-forest-labs/flux-1-schnell");
  });

  it("uses shared FLUX 2 Klein multipart controls on a Discover-ready 16:9 canvas", () => {
    expect(cloudflareSource).toContain('form.append("guidance", "5.5")');
    expect(cloudflareSource).toContain('form.append("width", "1280")');
    expect(cloudflareSource).toContain('form.append("height", "720")');
