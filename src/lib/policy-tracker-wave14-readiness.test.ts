...
import {
  POLICY_TRACKER_WAVE14_UPGRADES,
  WAVE14_INDEXABLE_POLICY_TRACKER_SLUGS,
} from "@/data/policy-tracker-wave14-upgrades";
import {
...
describe("wave 14 policy tracker readiness", () => {
  it("keeps the publication cohort explicit", () => {
    expect([...WAVE14_INDEXABLE_POLICY_TRACKER_SLUGS].sort()).toEqual([...EXPECTED].sort());
  });

  it.each(EXPECTED)("%s genuinely clears the unchanged canonical tracker gate", (slug) => {
    const tracker = POLICY_TRACKER_WAVE14_UPGRADES[slug];
