...
import {
  POLICY_TRACKER_WAVE8_UPGRADES,
  WAVE8_INDEXABLE_POLICY_TRACKER_SLUGS,
} from "@/data/policy-tracker-wave8-upgrades";
import {
...
const EXPECTED = [
  "regulatory-reform-treo",
  "career-technical-workforce",
  "semiconductor-manufacturing",
] as const;

describe("wave 8 policy tracker readiness", () => {
  it("keeps the publication cohort explicit", () => {
    expect([...WAVE8_INDEXABLE_POLICY_TRACKER_SLUGS].sort()).toEqual([...EXPECTED].sort());
  });

  it.each(EXPECTED)("%s genuinely clears the unchanged canonical tracker gate", (slug) => {
    const tracker = POLICY_TRACKER_WAVE8_UPGRADES[slug];
