...
import {
  POLICY_TRACKER_WAVE5_UPGRADES,
  WAVE5_INDEXABLE_POLICY_TRACKER_SLUGS,
} from "@/data/policy-tracker-wave5-upgrades";
import {
...
describe("wave 5 policy tracker readiness", () => {
  it("keeps the publication cohort explicit", () => {
    expect([...WAVE5_INDEXABLE_POLICY_TRACKER_SLUGS].sort()).toEqual([...EXPECTED].sort());
  });

  it.each(EXPECTED)("%s genuinely clears the unchanged canonical tracker gate", (slug) => {
    const tracker = POLICY_TRACKER_WAVE5_UPGRADES[slug];
