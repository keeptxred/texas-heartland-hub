...
import {
  POLICY_TRACKER_WAVE7_UPGRADES,
  WAVE7_INDEXABLE_POLICY_TRACKER_SLUGS,
} from "@/data/policy-tracker-wave7-upgrades";
import {
...
const EXPECTED = [
  "e-verify-employment",
  "public-sector-labor",
  "occupational-licensing-mobility",
] as const;

describe("wave 7 policy tracker readiness", () => {
  it("keeps the publication cohort explicit", () => {
    expect([...WAVE7_INDEXABLE_POLICY_TRACKER_SLUGS].sort()).toEqual([...EXPECTED].sort());
  });

  it.each(EXPECTED)("%s genuinely clears the unchanged canonical tracker gate", (slug) => {
    const tracker = POLICY_TRACKER_WAVE7_UPGRADES[slug];
