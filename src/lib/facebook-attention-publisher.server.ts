...
async function recordAttentionPost(
  ...
  const { error: queueError } = await db.from("publishing_queue").insert({
    ...
    notes: externalId
      ? `Facebook post ${externalId}; kind=attention-post; asset=openai-generated${providerNote}`
      : `KeepTXRed Facebook post; kind=attention-post; asset=openai-generated${providerNote}`,
  });
...
export async function publishKtrFacebookAttentionPost(args: {
  ...
  if (!args.suppliedImage) {
    return Response.json({
      ...
      image_prompt: prompt,
      reason: "OpenAI-generated image required before Facebook publication",
    ...
    return Response.json(
      {
        ok: false,
        posted: false,
        error: "Supplied OpenAI image does not match the selected attention post",
    ...
    return Response.json(
      {
        ok: false,
        posted: false,
        error: "Facebook attention post blocked because the OpenAI-generated image was invalid or could not be stored",
        detail: error instanceof Error ? error.message : String(error),
      },
      { status: 503 },
    );
  ...
  return Response.json({
    ...
    image_url: image.url,
    image_generated_by_openai: true,
