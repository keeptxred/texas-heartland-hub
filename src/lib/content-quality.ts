...
const STOCK_PENALTY_HOSTS = [
  "images.unsplash.com/photo-placeholder", "via.placeholder.com", "placehold.co", "picsum.photos",
];
...
export function scoreImage(input: {
  ...
  if (title && TEXAS_IMAGE_HINTS.some((h) => url.includes(h) && title.includes(h.replace(/[-]/g, " ")))) score += 10;
...
export type Region = "statewide" | "houston" | "dfw" | "austin" | "san-antonio" | "el-paso" | "rural";
...
export function classifyRegions(text: string): Region[] {
  const out: Region[] = [];
  for (const [region, re] of Object.entries(REGION_KEYWORDS) as [Region, RegExp][]) if (re.test(text)) out.push(region);
  return out;
}
...
export function buildTexasImpactSummary(text: string, regions: Region[]): string {
  ...
  const readable: Record<Region, string> = {
    statewide: "statewide", houston: "the Houston metro", dfw: "Dallas–Fort Worth",
    austin: "the Austin area", "san-antonio": "San Antonio", "el-paso": "El Paso", rural: "rural Texas",
  };
...
function bodyToText(row: QualityRow): string {
  ...
  try {
    const b = row.body_json as { intro?: string[]; sections?: { heading?: string; paragraphs?: string[]; bullets?: string[] }[] } | null | undefined;
    const parts: string[] = [];
...
export function scoreQuality(row: QualityRow): QualityResult {
  ...
  if (has(/texas|houston|dallas|austin|san antonio|fort worth|lone star/i)) score += 5; else flags.push("missing_texas_context");
  return { score: Math.max(0, Math.min(100, score)), flags };
}

export type AffiliateCategory = "moving" | "homes" | "insurance" | "energy" | "business" | "travel" | "products" | "services";
