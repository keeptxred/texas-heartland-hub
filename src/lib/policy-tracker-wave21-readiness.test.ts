...
describe("wave 21 state budget tracker readiness", () => {
  ...
  it("publishes the upgraded tracker through the canonical registry", () => {
    const live = getAnyPolicyTracker("state-budget");
    expect(live).toBe(POLICY_TRACKER_WAVE21_UPGRADE);
    expect(live?.updated).toBe("2026-09-09");
    expect(isPolicyTrackerIndexable(live)).toBe(true);
  });
});
