...
import {
  POLICY_TRACKER_WAVE15_UPGRADES,
  WAVE15_INDEXABLE_POLICY_TRACKER_SLUGS,
} from "@/data/policy-tracker-wave15-upgrades";
import {
...
describe("wave 15 policy tracker readiness", () => {
  it("keeps the publication cohort explicit", () => {
    expect([...WAVE15_INDEXABLE_POLICY_TRACKER_SLUGS].sort()).toEqual([...EXPECTED].sort());
  });

  it.each(EXPECTED)("%s genuinely clears the unchanged canonical tracker gate", (slug) => {
    const tracker = POLICY_TRACKER_WAVE15_UPGRADES[slug];
