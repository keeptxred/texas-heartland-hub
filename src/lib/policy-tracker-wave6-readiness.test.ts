...
import {
  POLICY_TRACKER_WAVE6_UPGRADES,
  WAVE6_INDEXABLE_POLICY_TRACKER_SLUGS,
} from "@/data/policy-tracker-wave6-upgrades";
import {
...
describe("wave 6 policy tracker readiness", () => {
  it("keeps the publication cohort explicit", () => {
    expect([...WAVE6_INDEXABLE_POLICY_TRACKER_SLUGS].sort()).toEqual([...EXPECTED].sort());
  });

  it.each(EXPECTED)("%s genuinely clears the unchanged canonical tracker gate", (slug) => {
    const tracker = POLICY_TRACKER_WAVE6_UPGRADES[slug];
