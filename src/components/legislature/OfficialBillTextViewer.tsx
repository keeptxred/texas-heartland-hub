...
export function OfficialBillTextViewer(props: Props) {
  const options = useMemo(() => {
    ...
    add("Introduced version", introducedBillTextUrl(props.sessionCode, props.billType, props.billNumber));
    return candidates;
  ...
  useEffect(() => {
    if (!options.some((option) => option.url === selectedUrl)) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok || typeof payload.text !== "string") {
        throw new Error(payload.error || "Unable to load the official bill text.");
      }
  ...
  return (
    <section className="scroll-mt-24 rounded-xl border bg-card p-6" id="bill-text">
      <div className="flex items-center gap-3">
        <FileText className="h-6 w-6 text-primary" />
        <h2 className="text-2xl font-bold">Official bill text</h2>
      </div>
      <p className="mt-3 text-sm text-muted-foreground">
        Read source text published by the Texas Legislature. KeepTXRed fetches it only when you
        request it, converts the official HTML to inert text for display, and does not rewrite it
        with AI.
      </p>
