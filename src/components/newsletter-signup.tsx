...
const NEWSLETTER_SIGNUP_LIVE = false;
...
export function NewsletterSignup({ sourcePage, compact = false }: Props) {
  ...
  const [articleBills, setArticleBills] = useState<ArticleBill[]>([]);

  useEffect(() => {
    const prefix = "/news/";
    if (!sourcePage?.startsWith(prefix)) return;
    const slug = sourcePage.slice(prefix.length).trim();
    if (!slug) return;

    let active = true;
    getApprovedBillsForArticleSlug(slug).then((bills) => {
      if (active) setArticleBills(bills);
    });
    return () => {
      active = false;
    };
  ...
  return (
    <>
      <ArticleBillsSection bills={articleBills} />
      {!NEWSLETTER_SIGNUP_LIVE && !sourcePage ? <span hidden data-newsletter-disabled-root /> : null}
      {NEWSLETTER_SIGNUP_LIVE ? (
        <section
          className={
            compact
