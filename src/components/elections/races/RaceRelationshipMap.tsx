...
export function RaceRelationshipMap({ race, candidates }: { race: RaceDetail; candidates: readonly CandidateSummary[] }) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-muted/30 p-6" aria-labelledby="race-relationship-heading">
      <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Entity relationships</p>
      <h2 id="race-relationship-heading" className="mt-2 text-2xl font-bold text-foreground">Election → candidate → district</h2>
      <p className="mt-2 max-w-3xl leading-7 text-muted-foreground">This map shows only relationships attached to the verified race record. Candidate profiles and district pages stay separate canonical entities so the same candidate or district can connect to other election records without duplicating content.</p>
      ...
      <p className="mt-5 text-xs leading-5 text-muted-foreground">District relationships are based on the race jurisdiction record, not an address lookup. Voters should use the official Texas voter or representation lookup for their specific address.</p>
    </section>
  );
}

function RelationshipNode({ eyebrow, title, detail, href }: { eyebrow: string; title: string; detail: string; href?: string }) {
  const content = <><span className="text-xs font-bold uppercase tracking-wide text-primary">{eyebrow}</span><strong className="mt-1 block text-lg text-foreground">{title}</strong><span className="mt-1 block text-sm text-muted-foreground">{detail}</span></>;
  return href ? <a href={href} className="block rounded-xl border border-border bg-card p-4 transition-colors hover:border-primary/40 hover:shadow-sm">{content}</a> : <div className="rounded-xl border border-border bg-card p-4">{content}</div>;
}

function Arrow() {
  return <span className="hidden text-center text-2xl font-bold text-muted-foreground/60 lg:block" aria-hidden="true">→</span>;
}
