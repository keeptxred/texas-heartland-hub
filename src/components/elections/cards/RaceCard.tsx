...
const STATUS_LABELS: Partial<Record<ElectionStatus, string>> = {
  scheduled: "Scheduled",
...
export function RaceCard({
  ...
  return (
    <article className={`rounded-xl border border-slate-200 bg-white p-5 shadow-sm ${className}`.trim()}>
      ...
      {(electionDate || electionType || status) && (
        <dl className="mt-5 grid gap-3 border-y border-slate-100 py-4 text-sm sm:grid-cols-3">
          {electionDate && (
            <div>
              <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">Election date</dt>
              <dd className="mt-1 font-semibold text-slate-900">{electionDate}</dd>
            </div>
          )}
          {electionType && (
            <div>
              <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">Election type</dt>
          ...
          {status && (
            <div>
              ...
              <dd className="mt-1 font-semibold text-slate-900">{STATUS_LABELS[status] ?? status}</dd>
            </div>
          )}
        </dl>
      )}
        ...
        <section aria-label={`${name} candidates`} className="mt-5">
          ...
          <ul className="mt-3 divide-y divide-slate-100 rounded-lg border border-slate-200">
            {candidates.map((candidate) => (
              <li key={candidate.id} className="flex items-center justify-between gap-3 px-4 py-3 text-sm">
                <div>
                  {candidate.candidateHref ? (
                    <a href={candidate.candidateHref} className="font-semibold text-slate-950 hover:text-red-700 hover:underline">
                      {candidate.name}
                    </a>
                  ) : (
                  ...
                  <div className="mt-1 text-xs text-slate-500">
                    {[candidate.partyLabel, candidate.incumbent ? "Incumbent" : null].filter(Boolean).join(" · ")}
                  </div>
                </div>
              </li>
            ))}
      ...
      <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3">
        <a href={raceHref} className="text-sm font-semibold text-red-700 underline-offset-4 hover:underline">
          View race details →
        </a>
