import type { PoliticalHistoryAuthorityPage as PoliticalHistoryPageData } from "@/data/texas-political-history-authority";
...
export function politicalHistoryAuthorityHead(page: PoliticalHistoryPageData) {
  ...
  const breadcrumbItems = [
    ...
    { name: "Texas Politics", item: `${SITE_URL}/texas-politics` },
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          author: {
            "@type": "Organization",
            name: "Keep TX Red Editorial Desk",
            url: `${SITE_URL}/about`,
          },
          publisher: {
            "@type": "Organization",
...
export function PoliticalHistoryAuthorityPage({ page }: { page: PoliticalHistoryPageData }) {
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <nav className="mb-6 text-sm text-muted-foreground" aria-label="Breadcrumb">
        <a href="/" className="hover:text-primary">Home</a>
        <span aria-hidden="true"> / </span>
        <a href="/texas-politics" className="hover:text-primary">Texas Politics</a>
        <span aria-hidden="true"> / </span>
      ...
      <header className="rounded-2xl border bg-card p-6 md:p-10">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <a href="/elections/2026" className="rounded-md bg-primary px-4 py-2 text-sm font-bold text-primary-foreground">
            Election Central
          </a>
          <a href="/texas-politics" className="rounded-md border px-4 py-2 text-sm font-bold hover:border-primary">
            Texas political history
          </a>
          <a href="/texas-government" className="rounded-md border px-4 py-2 text-sm font-bold hover:border-primary">
            Texas government
          </a>
        </div>
      </header>
          ...
          <table className="w-full min-w-[760px] text-left">
            <thead className="border-b bg-muted/40 text-sm">
              <tr>
                ...
                <th className="px-5 py-4 font-bold">Why it mattered</th>
              </tr>
            </thead>
            <tbody>
              {page.timeline.map((item) => (
                <tr key={`${item.year}-${item.event}`} className="border-b align-top last:border-b-0">
        ...
        <div className="mt-6 space-y-6">
          {page.faqs.map((faq) => (
            <div key={faq.question}>
              ...
              <p className="mt-2 leading-7 text-foreground/90">{faq.answer}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mt-12 rounded-2xl border bg-card p-6 md:p-8" aria-labelledby={`${page.slug}-sources`}>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Primary and institutional sources</p>
            <h2 id={`${page.slug}-sources`} className="mt-2 text-3xl font-bold">Sources and further reading</h2>
          </div>
          <p className="text-sm text-muted-foreground">Reviewed {page.reviewed}</p>
        </div>
        <ul className="mt-6 space-y-3">
          {page.sources.map((source) => (
            <li key={source.href}>
              <a href={source.href} target="_blank" rel="noreferrer" className="font-semibold text-primary underline underline-offset-4">
                {source.label}
              </a>
            </li>
          ))}
