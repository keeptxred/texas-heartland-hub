...
import type { ExploreEntityCard, ExploreGeographyPage } from "@/types/explore/public";
...
function Section({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return (
    <section aria-labelledby={id} className="scroll-mt-24">
      <h2 id={id} className="font-display text-3xl md:text-4xl">{title}</h2>
...
function ResourceCard({ title, description, href }: { title: string; description: string; href: string }) {
  return (
    <a href={href} className="rounded-lg border p-5 transition-colors hover:border-primary hover:bg-primary/5">
...
export function CountyAuthorityPage({ data }: { data: ExploreGeographyPage }) {
  ...
  const faq = [
    ...
    county ? { question: `What is the ${label} property-tax rate?`, answer: `The county rate in the KeepTXRed dataset is ${county.countyRate.toFixed(4)} per $100 of taxable value for tax year ${county.taxYear}. A total property-tax bill can also include city, school, college, hospital, utility, emergency-services, and other special-district rates.` } : null,
    ...
    "@graph": [
      {
        ...
        "@id": `https://keeptxred.com${canonicalPath}#webpage`,
        url: `https://keeptxred.com${canonicalPath}`,
        name: `${label}, Texas authority guide`,
        description: `County guide to demographics, economy, schools, taxes, districts, representatives, cities, parks, attractions, elections, and news for ${label}, Texas.`,
        about: { "@id": `https://keeptxred.com${canonicalPath}#county` },
      },
      {
        "@type": "AdministrativeArea",
        "@id": `https://keeptxred.com${canonicalPath}#county`,
        name: `${label}, Texas`,
        url: `https://keeptxred.com${canonicalPath}`,
      ...
      },
      {
        "@type": "BreadcrumbList",
        itemListElement: [
          { "@type": "ListItem", position: 1, name: "Explore Texas", item: "https://keeptxred.com/explore" },
          { "@type": "ListItem", position: 2, name: label, item: `https://keeptxred.com${canonicalPath}` },
        ],
      },
      data.items.length ? {
        "@type": "ItemList",
        name: `Parks and attractions in ${label}`,
        numberOfItems: data.items.length,
        itemListElement: data.items.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: item.name, url: `https://keeptxred.com/explore/${item.slug}` })),
      } : null,
      county ? {
        "@type": "Dataset",
        name: `${label} property-tax data`,
        description: `County, school-district, city-average, and special-district property-tax data for ${label}.`,
        temporalCoverage: String(county.taxYear),
        dateModified: TAX_RATE_DATASET.lastUpdated,
        creator: { "@type": "Organization", name: county.dataSource },
      } : null,
      faq.length ? {
        "@type": "FAQPage",
  ...
  return (
    <main>
      ...
      <div className="mx-auto max-w-6xl space-y-16 px-4 py-14">
        ...
        <Section id="demographics" title={`Demographics of ${label}`}><div className="grid gap-4 md:grid-cols-2"><ResourceCard title="Population and household profile" description="County population, age, income, poverty, education, housing, and veteran figures should be read with their Census or ACS estimate year." href={`https://www.census.gov/quickfacts/fact/table/${data.name.toLowerCase().replaceAll(" ", "")}countytexas/PST045225`} /><ResourceCard title="Data-quality standard" description="KeepTXRed does not display an unlabeled estimate as a current fact. Missing county values remain hidden until a verified source year is attached." href="https://www.census.gov/programs-surveys/acs" /></div></Section>

        <Section id="economy" title={`${label} economy`}><div className="grid gap-4 md:grid-cols-2"><ResourceCard title="Employment and labor force" description="Review county employment, unemployment, earnings, industry, and establishment data using the latest published reporting period." href="https://www.bls.gov/eag/" /><ResourceCard title="Regional economic context" description={`${label} is part of the ${county?.region || "Texas"} region. Local economic coverage is connected through KeepTXRed business and Texas economy reporting.`} href="/texas-economy" /></div></Section>

        <Section id="employers" title={`Major employers in ${label}`}><p className="max-w-4xl leading-7 text-muted-foreground">Employer rankings are shown only when an official county, city, workforce-board, or economic-development source supplies a verifiable list and reporting year. The page intentionally avoids invented employment counts.</p></Section>

        <Section id="schools" title={`School districts and education`}><div className="grid gap-4 md:grid-cols-2">{schoolDistricts.length ? schoolDistricts.map((district) => <div key={district.name} className="rounded-lg border p-5"><GraduationCap className="size-5 text-primary"/><h3 className="mt-3 font-semibold">{district.name}</h3><p className="mt-2 text-sm text-muted-foreground">2025 tax rate: {district.rate.toFixed(4)} per $100 of taxable value</p></div>) : <p className="text-muted-foreground">No school-district tax records are currently attached to this county profile. Cross-county districts are added only after their service relationship is verified.</p>}{schools.map((school) => <Link key={school.id} to="/explore/$slug" params={{ slug: school.slug }} className="rounded-lg border p-5 hover:border-primary"><h3 className="font-semibold">{school.name}</h3><p className="mt-2 text-sm text-muted-foreground">{school.summary}</p></Link>)}</div></Section>

        <Section id="property-taxes" title={`${label} property taxes`}>{county ? <div className="overflow-x-auto rounded-lg border"><table className="w-full min-w-[620px] text-left text-sm"><caption className="sr-only">{label} tax rates for {county.taxYear}</caption><thead className="bg-muted/40"><tr><th className="p-4">Taxing layer</th><th className="p-4">Rate per $100</th><th className="p-4">Year</th></tr></thead><tbody><tr className="border-t"><td className="p-4">County</td><td className="p-4">{county.countyRate.toFixed(4)}</td><td className="p-4">{county.taxYear}</td></tr><tr className="border-t"><td className="p-4">Average city component</td><td className="p-4">{county.cityAvgRate.toFixed(4)}</td><td className="p-4">{county.taxYear}</td></tr><tr className="border-t"><td className="p-4">Average special-district component</td><td className="p-4">{county.specialDistrictRate.toFixed(4)}</td><td className="p-4">{county.taxYear}</td></tr></tbody></table><p className="border-t p-4 text-sm text-muted-foreground">These components do not equal every homeowner’s final rate. City, ISD, hospital, college, MUD, PID, ESD, and other local rates vary by address.</p></div> : <p className="text-muted-foreground">Verified county tax data is not currently available.</p>}</Section>

        <Section id="legislative-districts" title={`Legislative districts overlapping ${label}`}><div className="grid gap-4 md:grid-cols-3"><ResourceCard title="Texas House districts" description="See current Texas House districts, members, and representative profiles." href="/legislature/house"/><ResourceCard title="Texas Senate districts" description="See current Texas Senate districts, members, and representative profiles." href="/legislature/senate"/><ResourceCard title="U.S. congressional districts" description="Open Election Central for congressional races, candidates, and results connected to Texas counties." href="/elections"/></div></Section>

        <Section id="representatives" title={`Representatives serving ${label}`}><div className="rounded-lg border p-6"><Users className="size-6 text-primary"/><p className="mt-3 leading-7 text-muted-foreground">Representative cards are connected through the Legislature and Election Central data models so officeholders are not duplicated on county pages. District-to-county relationships may include multiple Texas House, Texas Senate, and U.S. House districts.</p><a href="/legislature" className="mt-4 inline-block font-semibold text-primary hover:underline">Browse current representatives</a></div></Section>

        <Section id="cities" title={`Cities and communities in ${label}`}>{cities.length ? <div className="flex flex-wrap gap-2">{cities.map((city) => <span key={city} className="rounded-full border px-4 py-2 text-sm">{city}</span>)}</div> : <p className="text-muted-foreground">No city relationships are currently attached to published Explore Texas records for this county.</p>}</Section>
        ...
        <Section id="elections" title={`${label} election history`}><div className="rounded-lg border p-6"><Vote className="size-6 text-primary"/><p className="mt-3 leading-7 text-muted-foreground">County election history belongs to the shared Election Central dataset so results can retain the election year, office, candidates, turnout, vote totals, and source. Future results are never hardcoded.</p><a href="/elections/results" className="mt-4 inline-block font-semibold text-primary hover:underline">View election results</a></div></Section>

        <Section id="news" title={`Related ${label} news`}><div className="rounded-lg border p-6"><Newspaper className="size-6 text-primary"/><p className="mt-3 leading-7 text-muted-foreground">Related coverage is matched by county, city, district, representative, and validated geographic metadata. Unpublished stories and articles without working internal slugs are excluded.</p><a href={`/search?q=${encodeURIComponent(label)}`} className="mt-4 inline-block font-semibold text-primary hover:underline">Search KeepTXRed coverage</a></div></Section>
        ...
        <section className="rounded-lg border bg-muted/20 p-6" aria-labelledby="sources"><h2 id="sources" className="font-display text-2xl">Sources and data notes</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Property-tax data: {county?.dataSource || "Texas Comptroller dataset"}, tax year {county?.taxYear || TAX_RATE_DATASET.taxYear}; dataset updated {TAX_RATE_DATASET.lastUpdated}. Destination, city, park, and attraction relationships come from verified Explore Texas records. Time-sensitive demographic, economic, district, representative, election, and news fields remain source-dated and are hidden when a verified relationship is unavailable.</p></section>
      </div>
    </main>
  );
}
