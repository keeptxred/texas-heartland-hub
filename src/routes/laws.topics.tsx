...
export const Route = createFileRoute("/laws/topics")({
  head: () => {
    ...
    return { meta: seo.meta, links: seo.links, scripts: [
      ...
      { type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@type": "ItemList", name: "Texas Law Topics", numberOfItems: LAW_TOPICS.length, itemListElement: LAW_TOPICS.map((topic, index) => ({ "@type": "ListItem", position: index + 1, name: topic.title, url: `${SITE_URL}/laws/topic/${topic.slug}` })) }) },
    ] };
  },
