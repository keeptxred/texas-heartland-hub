    ...
    <section className="border-t bg-white" aria-labelledby="priority-election-pages">
      <div className="mx-auto max-w-[1200px] px-6 py-12">
        ...
        {PRIORITY_CANDIDATES.length ? (
          <div className="mt-8">
            <div className="flex items-end justify-between gap-4">
              <h3 className="text-lg font-bold text-slate-950">Verified candidate profiles</h3>
              <Link to="/elections/candidates" className="text-sm font-semibold text-red-700 hover:underline">Browse all candidates →</Link>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {PRIORITY_CANDIDATES.map((candidate) => (
                <Link key={candidate.id} to="/elections/candidates/$candidateSlug" params={{ candidateSlug: candidate.slug }} className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 font-semibold text-slate-900 hover:border-red-300 hover:text-red-700">{candidate.fullName}</Link>
              ))}
            </div>
          </div>
        ) : null}
        ...
        {PRIORITY_DISTRICTS.length ? (
          <div className="mt-8">
            <div className="flex items-end justify-between gap-4">
              <h3 className="text-lg font-bold text-slate-950">Active 2026 district pages</h3>
              <Link to="/elections/districts" className="text-sm font-semibold text-red-700 hover:underline">Browse all districts →</Link>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {PRIORITY_DISTRICTS.map((district) => (
                <Link key={district.slug} to="/elections/districts/$districtSlug" params={{ districtSlug: district.slug }} className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 font-semibold text-slate-900 hover:border-red-300 hover:text-red-700">{district.label}</Link>
              ))}
            </div>
          </div>
        ) : null}
