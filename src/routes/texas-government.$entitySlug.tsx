    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
        <main className="space-y-8">
          ...
          <AuthoritySection id="responsibilities" title="Constitutional and statutory responsibilities" icon={<Scale className="h-6 w-6 text-primary"/>}><p className="leading-7">{entity.constitutionalResponsibilities}</p><h3 className="mt-5 font-bold">Primary legal authority</h3><ul className="mt-3 space-y-2">{entity.constitutionalBasis.map((basis) => <li key={basis} className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-primary"/><span>{basis}</span></li>)}</ul></AuthoritySection>

          <AuthoritySection id="leadership" title="Current officeholder or leadership" icon={<Users className="h-6 w-6 text-primary"/>}><div className="rounded-xl border bg-muted/30 p-5"><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">{entity.officeholderTitle}</p><h3 className="mt-2 text-2xl font-bold">{entity.currentOfficeholder}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{entity.officeholderNote}</p><div className="mt-4 flex flex-wrap gap-3">{entity.relatedRepresentatives.slice(0, 2).map((link) => <AuthorityLink key={link.href} link={link}/>)}<a href={entity.officialUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm font-bold text-primary hover:underline">Official leadership source <ExternalLink className="h-4 w-4"/></a></div></div></AuthoritySection>

          <AuthoritySection id="history" title="Historical information" icon={<Landmark className="h-6 w-6 text-primary"/>}><ol className="space-y-4">{entity.history.map((item, index) => <li key={item} className="relative border-l-2 border-border pb-4 pl-6 last:pb-0"><span className="absolute -left-[7px] top-1 h-3 w-3 rounded-full bg-primary"/><span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Milestone {index + 1}</span><p className="mt-1 leading-7">{item}</p></li>)}</ol></AuthoritySection>

          <AuthoritySection id="powers" title="Important powers" icon={<Gavel className="h-6 w-6 text-primary"/>}><div className="grid gap-3 sm:grid-cols-2">{entity.powers.map((power) => <div key={power} className="rounded-lg border p-4"><p className="font-medium leading-6">{power}</p></div>)}</div><h3 className="mt-6 text-lg font-bold">What this entity cannot do</h3><ul className="mt-3 space-y-2 text-muted-foreground">{entity.limitations.map((item) => <li key={item}>• {item}</li>)}</ul></AuthoritySection>
          ...
          <AuthoritySection id="news" title={`Latest ${entity.shortName} news`} icon={<Landmark className="h-6 w-6 text-primary"/>}>
            {news.length ? <div className="grid gap-4 sm:grid-cols-2">{news.map((article) => <a key={article.slug} href={`/article/${article.slug}`} className="rounded-xl border p-5 hover:border-primary"><p className="text-xs font-bold uppercase tracking-wide text-primary">{article.category}</p><h3 className="mt-2 font-bold leading-snug">{article.title}</h3><p className="mt-2 line-clamp-3 text-sm leading-6 text-muted-foreground">{article.dek}</p><time className="mt-3 block text-xs text-muted-foreground">{new Date(article.publishedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</time></a>)}</div> : <div className="rounded-lg bg-muted/40 p-5"><p className="text-muted-foreground">No matching published KeepTXRed articles are currently linked to this entity.</p><a href="/politics" className="mt-3 inline-flex text-sm font-bold text-primary hover:underline">Browse the latest Texas politics coverage →</a></div>}
          </AuthoritySection>

          <AuthoritySection id="faqs" title="Frequently asked questions" icon={<Scale className="h-6 w-6 text-primary"/>}><div className="divide-y rounded-xl border">{entity.faqs.map((faq) => <details key={faq.question} className="group p-5"><summary className="cursor-pointer list-none pr-8 font-bold marker:hidden">{faq.question}<span className="float-right text-primary group-open:rotate-45">+</span></summary><p className="mt-3 max-w-3xl leading-7 text-muted-foreground">{faq.answer}</p></details>)}</div></AuthoritySection>

          <AuthoritySection id="related-government" title="Related Texas government entities" icon={<Landmark className="h-6 w-6 text-primary"/>}><div className="grid gap-3 sm:grid-cols-2">{relatedEntities.map((related) => related ? <a key={related.slug} href={governmentPath(related.slug)} className="rounded-lg border p-4 hover:border-primary"><p className="font-bold">{related.name}</p><p className="mt-1 text-sm text-muted-foreground">{related.branch} · {related.entityType}</p></a> : null)}</div></AuthoritySection>
        </main>
...
function AuthoritySection({ id, title, icon, children }: { id: string; title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return <section id={id} className="scroll-mt-24 rounded-xl border bg-card p-6"><div className="flex items-center gap-3">{icon}<h2 className="text-2xl font-bold">{title}</h2></div><div className="mt-5">{children}</div></section>;
}

function LinkGrid({ links }: { links: GovernmentLink[] }) {
  return links.length ? <div className="grid gap-3 sm:grid-cols-2">{links.map((link) => <a key={`${link.href}-${link.label}`} href={link.href} className="rounded-lg border p-4 hover:border-primary"><p className="font-bold">{link.label}</p>{link.description && <p className="mt-1 text-sm leading-6 text-muted-foreground">{link.description}</p>}</a>)}</div> : <p className="text-muted-foreground">No directly related records are currently linked.</p>;
}
