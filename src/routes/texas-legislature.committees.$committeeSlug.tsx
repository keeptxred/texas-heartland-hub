  ...
  loader: async ({ params }) => {
    ...
    if (historyResult.status === 'fulfilled') {
      if (historyResult.value.error) {
        console.error(
          `Committee bill history failed for ${committee.committee_slug}:`,
          historyResult.value.error.message ?? historyResult.value.error,
        );
      } else {
    ...
    if (relatedContentResult.status === 'fulfilled') {
      ...
      console.error(
        `Committee related content failed for ${committee.committee_slug}:`,
        relatedContentResult.reason,
      );
    }
