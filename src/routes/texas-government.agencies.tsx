...
export const Route = createFileRoute('/texas-government/agencies')({
  head: () => ({
    meta: [
      { title: 'Texas State Agency Directory | Official Agency & Department Links' },
      { name: 'description', content: 'Find official Texas state agency and department directories, service categories and verified entry points for common state services.' },
      { name: 'robots', content: 'index, follow, max-image-preview:large' },
      { property: 'og:title', content: 'Texas State Agency Directory' },
    ...
    scripts: [{ type: 'application/ld+json', children: JSON.stringify({
      ...
      '@type': 'CollectionPage',
      name: 'Texas State Agency Directory',
      url: CANONICAL,
      dateModified: '2026-08-11',
      isBasedOn: [TEXAS_GOV_DIRECTORY, TSL_DIRECTORY],
      mainEntity: {
        '@type': 'ItemList',
        numberOfItems: VERIFIED_ENTRY_POINTS.length,
        itemListElement: VERIFIED_ENTRY_POINTS.map((agency, index) => ({ '@type': 'ListItem', position: index + 1, name: agency.name, url: agency.href, description: agency.use })),
      },
    }).replace(/</g, '\\u003c') }],
  }),
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <header className="mt-6 border-b pb-8">
        ...
        <h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">Texas State Agency Directory</h1>
        <p className="mt-4 max-w-4xl text-lg leading-8 text-muted-foreground">Texas.gov currently catalogs more than 150 state agencies and departments. KeepTXRed uses the official Texas.gov and Texas State Library directories as the complete discovery layer rather than copying a large agency list that can silently go stale.</p>
      </header>
      ...
      <CitationTrustPanel
        ...
        methodology="KeepTXRed does not mirror all state-agency contact records. The complete list remains delegated to the official live directories, while this page supplies stable service-area context and a small set of directly verified high-use entry points."
