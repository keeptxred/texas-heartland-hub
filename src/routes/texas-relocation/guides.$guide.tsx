...
function RelocationGuidePage() {
  ...
  const calculators = guide.relatedCalculatorPaths
    .map((path) => getCalculatorBySlug(path))
    .filter(Boolean);
    ...
    <main>
      ...
      <section className="mx-auto max-w-5xl px-4 py-12">
        ...
        <section className="mt-12">
          <h2 className="font-display text-3xl">Use these planning tools</h2>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {calculators.map((calculator) => calculator && (
              <a key={calculator.slug} href={calculator.slug} className="border border-border bg-card p-5 hover:border-primary">
        ...
        <section className="mt-12 border border-border bg-muted p-6">
          <h2 className="font-display text-2xl">Explore the initial county guides</h2>
          <div className="mt-4 flex flex-wrap gap-3">
            {LAUNCH_COUNTIES.map((county) => (
              <a key={county.slug} href={`${RELOCATION_LAUNCH_PATH}/${county.slug}`} className="border border-border bg-background px-3 py-2 text-sm hover:border-primary">
                {county.name}
              </a>
            ))}
          </div>
