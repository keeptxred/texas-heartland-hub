        ...
        {POLICY_TOOLS.map((tool) => (
          <article key={tool.href} className="flex flex-col border bg-muted/20 p-6">
            ...
            <div className="mt-6 space-y-3 border-t pt-4">
              <Link to={tool.href} className="block font-semibold text-primary hover:underline">Open tool →</Link>
              <Link to={tool.guideHref} className="block text-sm font-semibold hover:text-primary">{tool.guideLabel} →</Link>
            </div>
          </article>
        ))}
      </section>
