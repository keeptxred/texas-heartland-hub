  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section className="mt-10">
        ...
        {bills.length ? <div className="mt-6 grid gap-4 lg:grid-cols-2">{bills.map((bill) => <article key={bill.id} className="rounded-xl border bg-card p-5 transition hover:border-primary"><div className="flex items-start justify-between gap-4"><div><Link to={canonicalBillPath(bill)} className="text-xl font-bold text-primary hover:underline">{bill.bill_identifier}</Link><p className="mt-2 font-medium leading-snug">{bill.caption}</p></div><span className="shrink-0 rounded-full bg-muted px-3 py-1 text-xs font-semibold">{bill.current_status_label}</span></div><div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground"><span>{bill.chamber === 'house' ? 'Texas House' : bill.chamber === 'senate' ? 'Texas Senate' : 'Joint'}</span>{bill.last_action_date && <span>Updated {new Date(`${bill.last_action_date}T12:00:00`).toLocaleDateString()}</span>}</div></article>)}</div> : <div className="mt-6 rounded-xl border border-dashed p-10 text-center"><h3 className="font-semibold">No matching bills</h3><p className="mt-2 text-sm text-muted-foreground">Clear the filters or search by bill number, caption, sponsor, committee or subject.</p></div>}
        {pages > 1 && <nav className="mt-8 flex items-center justify-center gap-3" aria-label="Bill results pages"><Link to="/bills" search={{ ...search, page: Math.max(1, search.page - 1) }} disabled={search.page <= 1} className="rounded-md border px-4 py-2 aria-disabled:opacity-50">Previous</Link><span className="text-sm">Page {search.page} of {pages}</span><Link to="/bills" search={{ ...search, page: Math.min(pages, search.page + 1) }} disabled={search.page >= pages} className="rounded-md border px-4 py-2 aria-disabled:opacity-50">Next</Link></nav>}
      </section>
    </div>
  );
}
