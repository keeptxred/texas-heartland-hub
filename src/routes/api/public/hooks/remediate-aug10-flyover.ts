...
const STORY_SPECS = [
  { key: "state-fair-gun-ban", terms: [/state fair/i, /(gun|handgun|firearm)/i] },
  { key: "fort-worth-alligators-shot", terms: [/(fort worth|nature center)/i, /alligator/i, /(shot|crossbow|killed)/i] },
  { key: "don-nelson", terms: [/don nelson/i, /(mavericks|dallas)/i] },
  { key: "ingram-school-flood-repairs", terms: [/ingram/i, /(school|campus)/i, /(flood|storm|repair)/i] },
  { key: "st-louis-encephalitis", terms: [/(st\. louis encephalitis|encephalitis)/i, /(corpus christi|kingsville|mosquito)/i] },
  { key: "lakeside-fentanyl-children", terms: [/fentanyl/i, /(lakeside|children|child endangerment)/i] },
  { key: "dallas-pedestrian-waymo", terms: [/dallas/i, /(waymo|pedestrian|maple avenue)/i] },
  { key: "bastrop-council-retreat", terms: [/bastrop/i, /(council|fredericksburg|gerdes)/i] },
  { key: "tate-taylor-sprint-double", terms: [/tate taylor/i, /(200 meters|sprint|under-20|texas tech)/i] },
  { key: "texas-stadium-mavericks-redevelopment", terms: [/(texas stadium|irving)/i, /(mavericks|redevelop)/i] },
  { key: "cowboys-quinnen-williams", terms: [/quinnen williams/i, /cowboys/i] },
  { key: "rangers-jonah-bride", terms: [/jonah bride/i, /(rangers|round rock|osuna|leiter)/i] },
  { key: "heb-store-upgrades", terms: [/(h-e-b|heb)/i, /(upgrade|remodel|5\.2 million)/i] },
  { key: "caseys-pak-a-sak", terms: [/(casey|casey's)/i, /pak-a-sak/i] },
  { key: "sushi-door-dash-dispute", terms: [/(sushi by the heights|sushi)/i, /(doordash|door dash)/i] },
  { key: "kris6-anchor-layoffs", terms: [/(kris 6|hofmann)/i, /(layoff|sign off|scripps)/i] },
] as const;
...
function parseGoogleFeed(xml: string) {
  ...
  return blocks.slice(0, 40).flatMap((block) => {
    ...
    return [{ title: title.slice(0, 500), link, description: description.slice(0, 1200), source, pub_date: new Date(Number.isNaN(parsed) ? Date.now() : parsed).toISOString() }];
  });
}
...
async function handler() {
  ...
  const fetchedRows: Array<{ title: string; link: string; description: string; source: string; pub_date: string }> = [];
  ...
  const { data: rows, error: readError } = await supabase
    ...
    .gte("pub_date", "2026-08-03T00:00:00Z")
  ...
  return Response.json({ ok: true, discoveryFetched: unique.length, feedErrors, results });
}
