...
const LIVE_TEST_SOURCE = "texasdefined_live_payment_test";
...
export const Route = createFileRoute("/api/public/ops/replay-live-payment-diagnostic")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const headers = {
        ...
        try {
          ...
          if (
            ...
            return Response.json(
              { ok: false, error: "Expected paid TexasDefined diagnostic session not found." },
              { status: 409, headers },
            );
          }
          ...
          console.error("TexasDefined live diagnostic replay failed", error);
          return Response.json(
            { ok: false, error: "Unable to replay TexasDefined live diagnostic event." },
            { status: 500, headers },
          );
        }
      },
