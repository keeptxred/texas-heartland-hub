...
import { LAUNCH_COUNTIES, RELOCATION_LAUNCH_PATH, getLaunchGuideBySlug } from "@/data/relocationLaunch";
...
export const Route = createFileRoute("/texas-relocation/guides/$guide")({
  loader: ({ params }) => {
    ...
    const guide = getLaunchGuideBySlug(routeParams.guide);
  ...
  head: ({ loaderData }) => {
    ...
    return {
      ...
      scripts: [{ type: "application/ld+json", children: JSON.stringify({
        "@context": "https://schema.org", "@type": "Article", headline: loaderData.title,
        description: loaderData.description, url,
...
function RelocationGuidePage() {
  ...
  const calculators = guide.relatedCalculatorPaths
    ...
    .filter((calculator: ReturnType<typeof getCalculatorBySlug>): calculator is Calculator => Boolean(calculator));

  return (
    <main>
      <section className="bg-secondary text-secondary-foreground"><div className="mx-auto max-w-5xl px-4 py-14">
        <a href={RELOCATION_LAUNCH_PATH} className="text-xs font-bold uppercase tracking-widest text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">Texas relocation center</a>
      ...
      <section className="mx-auto max-w-5xl px-4 py-12" aria-labelledby="guide-sections-heading">
        ...
        <div className="grid gap-6 md:grid-cols-3">{sections.map((section) => <article key={section.heading} className="border border-border bg-card p-6"><h3 className="font-display text-2xl">{section.heading}</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{section.body}</p></article>)}</div>

        <section className="mt-12" aria-labelledby="planning-tools-heading"><h2 id="planning-tools-heading" className="font-display text-3xl">Use these planning tools</h2><div className="mt-5 grid gap-4 md:grid-cols-2">{calculators.map((calculator: Calculator) => <a key={calculator.slug} href={calculator.slug} className="border border-border bg-card p-5 hover:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><h3 className="font-display text-xl">{calculator.title}</h3><p className="mt-2 text-sm text-muted-foreground">{calculator.description}</p></a>)}</div></section>

        <section className="mt-12 border border-border bg-muted p-6" aria-labelledby="county-guides-heading"><h2 id="county-guides-heading" className="font-display text-2xl">Explore the initial county guides</h2><nav aria-label="Texas county relocation guides" className="mt-4 flex flex-wrap gap-3">{LAUNCH_COUNTIES.map((county) => <a key={county.slug} href={`${RELOCATION_LAUNCH_PATH}/${county.slug}`} className="border border-border bg-background px-3 py-2 text-sm hover:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">{county.name}</a>)}</nav></section>
