...
function DmvHubPage() {
  return (
    <main>
      ...
      <section className="border-t bg-muted/20"><div className="mx-auto max-w-4xl px-4 py-10 text-sm text-muted-foreground"><p>Official requirements can change. Confirm forms, taxes, bond amounts, lien releases, permit and plate fees, salvage-title eligibility, ownership evidence, inspections, office hours, appointment availability, emissions rules, training, and transaction requirements with the relevant Texas or federal agency.</p></div></section>
    </main>
  );
}
...
export const Route = createFileRoute("/dmv")({
  head: () => {
    const seo = buildSeo({ title: "Texas DMV & DPS: Bonded Titles, Permits, Plates & Licenses", description: "Texas DMV and DPS guides for bonded titles, salvage and rebuilt titles, temporary permits, license plates, registration, driver licenses, CDL, ID cards, and appointments.", path: "/dmv", type: "website", keywords: "Texas DMV, Texas bonded title, Texas salvage title, Texas rebuilt title, Texas temporary permits, Texas license plates, Texas vehicle registration, Texas driver license" });
    return { meta: seo.meta, links: seo.links, scripts: [
      { type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@type": "CollectionPage", name: "Texas DMV & Driver Services", url: `${SITE_URL}/dmv`, description: "Texas guides for bonded titles, salvage and rebuilt titles, temporary permits, license plates, vehicle registration, driver licenses, commercial licenses, identification cards, DPS appointments, and REAL ID.", isPartOf: { "@type": "WebSite", name: "Keep TX Red", url: `${SITE_URL}/` } }) },
