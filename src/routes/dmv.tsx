...
function DmvHubPage() {
  return (
    <main>
      ...
      <section className="border-y bg-muted/30"><div className="mx-auto max-w-4xl px-4 py-14"><h2 className="text-3xl font-bold">Quick agency guide</h2><div className="mt-7 overflow-x-auto rounded-xl border bg-card"><table className="w-full text-left text-sm"><thead className="border-b bg-muted/50"><tr><th className="px-4 py-3 font-semibold">Task</th><th className="px-4 py-3 font-semibold">Start with</th></tr></thead><tbody className="divide-y">{[["Driver license, ID card, REAL ID, CDL, driving test", "Texas DPS"],["Apply for a commercial learner permit or CDL", "Texas DPS and FMCSA"],["Apply for a non-driver Texas identification card", "Texas DPS"],["Check license eligibility, holds, or reinstatement items", "DPS License Eligibility"],["Schedule an in-person driver license or ID service", "Texas Scheduler"],["Renew, replace, or change a driver license or ID address", "Texas DPS or TxT"],["Vehicle title, registration, plates, disabled placard", "County tax assessor-collector"],["Dealer licensing, motor carriers, statewide vehicle programs", "Texas DMV"],["New resident with a vehicle", "County tax office first, then DPS"]].map(([task, office]) => <tr key={task}><td className="px-4 py-3">{task}</td><td className="px-4 py-3 font-semibold">{office}</td></tr>)}</tbody></table></div></div></section>
      ...
      <section className="border-t bg-muted/20"><div className="mx-auto max-w-4xl px-4 py-10 text-sm text-muted-foreground"><p>Official requirements can change. Confirm forms, fees, eligibility, office hours, appointment availability, medical certification, training, and transaction rules with the relevant Texas or federal agency before traveling or operating a vehicle.</p></div></section>
    </main>
  );
}
...
export const Route = createFileRoute("/dmv")({
  head: () => {
    const seo = buildSeo({ title: "Texas DMV & DPS: Driver Licenses, CDL, ID Cards & Appointments", description: "Texas DMV and DPS guides for driver licenses, commercial licenses, ID cards, license status, appointments, REAL ID, registration, titles, plates, and offices.", path: "/dmv", type: "website", keywords: "Texas DMV, Texas CDL, Texas commercial driver license, Texas identification card, check Texas driver license status, Texas DPS appointment, Texas REAL ID, Texas vehicle registration" });
    return { meta: seo.meta, links: seo.links, scripts: [
      { type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@type": "CollectionPage", name: "Texas DMV & Driver Services", url: `${SITE_URL}/dmv`, description: "Texas guides for driver licenses, commercial licenses, identification cards, license status, DPS appointments, REAL ID, vehicle registration, titles, and plates.", isPartOf: { "@type": "WebSite", name: "Keep TX Red", url: `${SITE_URL}/` } }) },
