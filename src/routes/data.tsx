...
export const Route = createFileRoute("/data")({
  head: () => {
    ...
    return { meta: seo.meta, links: seo.links, scripts: [
      ...
      { type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@type": "ItemList", name: "Texas Data Center", numberOfItems: ALL_DATA_SETS.length, itemListElement: ALL_DATA_SETS.map((dataset, index) => ({ "@type": "ListItem", position: index + 1, name: dataset.title, url: `${SITE_URL}/data/${dataset.slug}` })) }) },
    ] };
  },
  component: DataHub,
});
