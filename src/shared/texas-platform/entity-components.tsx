...
export function SharedCollectionLinks({ collections, title = 'Explore related sections' }: { collections: SharedEntityCollection[]; title?: string }) {
    ...
    <section className="rounded-xl border bg-muted/20 p-6">
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        {unique.map((collection) => (
          <Link key={collection.id} to={collection.route} className="rounded-full border bg-background px-4 py-2 text-sm font-semibold hover:border-primary hover:text-primary">
            {collection.title} ({collection.entities.length})
          </Link>
        ))}
      </div>
    </section>
...
export function SharedEntityDetails({
  ...
  collections?: SharedEntityCollection[];
}) {
  return (
    <div className="space-y-10">
      ...
      <SharedCollectionLinks collections={collections} />
